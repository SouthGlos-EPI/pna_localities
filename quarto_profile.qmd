...
**Figure X:** `r locality` PNA Locality population breakdown by age & sex (`r max(pops$year)`).
```{r}
...
**Table X:** Number and rate of pharmacies and items dispensed in `r locality` PNA Locality, `r unique(first(disp_data_loc$fy))` - `r unique(last(disp_data_loc$fy))`.
...
*Notes: Populations for locality and local authority are based on ONS mid-year population estimates. The population for each financial year is taken as the mid-year estimate for the first of the two years that make up the financial year. For example, for 2021/22 the population is taken as the mid-year estimate for 2021. If that year is not yet available, the latest available year is used.*
...
# Compute values for text
last_full_fy <- unique(last(filter(disp_data_loc, n_months == 12)$fy))
loc_n_nms <- format(filter(disp_data_loc, fy == last_full_fy, area == locality)$n_nms, big.mark = ",")
...
**New Medicine Service** (NMS) accreditation: `r loc_pharm_nms` `r pluralise(loc_pharm_nms, "pharmacy")`. In `r last_full_fy` (the last full year of data), there were `r loc_n_nms` NMSs undertaken.
